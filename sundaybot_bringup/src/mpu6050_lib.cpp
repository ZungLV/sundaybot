...
#include "sundaybot_bringup/mpu6050_lib.h"
...
MPU6050::MPU6050(int8_t addr, bool run_update_thread) {
	...
	i2c_smbus_write_byte_data(f_dev, 0x6b, 0x00); //Take MPU6050 out of sleep mode - see Register Map

	i2c_smbus_write_byte_data(f_dev, 0x1a, 0x03); //Set DLPF (low pass filter) to 44Hz (so no noise above 44Hz will pass through)

	i2c_smbus_write_byte_data(f_dev, 0x19, 0x04); //Set sample rate divider (to 200Hz) - sample rate = gyroscope output rate/(1 + SMPLRT_DIV)

	i2c_smbus_write_byte_data(f_dev, 0x1b, 0x00); //Configure gyroscope settings - see Register Map (see MPU6050.h for the GYRO_CONFIG parameter)

	i2c_smbus_write_byte_data(f_dev, 0x1c, 0x00); //Configure accelerometer settings - see Register Map (see MPU6050.h for the GYRO_CONFIG parameter)

	i2c_smbus_write_byte_data(f_dev, 0x1d, 0x03); //Configure accelerometer settings 2 - see Register Map (see MPU6050.h for the GYRO_CONFIG parameter)

	i2c_smbus_write_byte_data(f_dev, 0x37, 0x22); // Configure Interrupts and Bypass Enable

	i2c_smbus_write_byte_data(f_dev, 0x37, 0x01); // Enable data ready (bit 0) interrupt

	//Set offsets to zero
	// i2c_smbus_write_byte_data(f_dev, 0x06, 0x00), i2c_smbus_write_byte_data(f_dev, 0x07, 0x00), i2c_smbus_write_byte_data(f_dev, 0x08, 0x00), i2c_smbus_write_byte_data(f_dev, 0x09, 0x00), i2c_smbus_write_byte_data(f_dev, 0x0A, 0x00), i2c_smbus_write_byte_data(f_dev, 0x0B, 0x00), i2c_smbus_write_byte_data(f_dev, 0x00, 0b10000001), i2c_smbus_write_byte_data(f_dev, 0x01, 0b00000001), i2c_smbus_write_byte_data(f_dev, 0x02, 0b10000001);
